...
function filterJobs() {
  ...
  jobCards.forEach((card) => {
    ...
    const matchType =
      activeJobType === "All Types" ||
      activeJobType === "All" ||
      activeJobType === null ||
      cardType === activeJobType;
    ...
    card.style.display = matchType && matchCategory ? "block" : "none";
  });
}
...
function renderJobs(jobs) {
  ...
  jobs.forEach((job) => {
    ...
    const type = job.jobType || "N/A";
    const salary = job.salary ? `$${job.salary}` : "Not disclosed";
    const category = job.skill || "General"; // fallback to skill if category missing
    const jobId = job.id;

    jobCard.setAttribute("data-type", type);
    jobCard.setAttribute("data-category", category);

    // ✅ Dynamically insert context path using JSP only once in HTML (not JS)
    const contextPath = window.location.origin + "/yourAppName"; // replace "yourAppName" with your real app context
    ...
    jobCard.innerHTML = `
      <div class="job-header">
        ...
        <div class="job-info">
          <h2>${title}</h2>
          <p class="company">${job.companyName || "Anonymous"}</p>
          <p class="details">
            <span>📍 ${location}</span> •
            <span>💼 ${type}</span> •
            <span>💰 ${salary}</span> •
      ...
      <!-- ✅ Corrected apply link -->
      <a href="${api}api/applications/apply/${jobId}" class="view-btn">Apply</a>
    `;
